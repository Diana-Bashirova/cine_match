...
def get_consensus_recommendations(request):
    """
    🔽 ИСПРАВЛЕНО: Совместные рекомендации с новыми весами:
    - 50% — критерии комнаты (жанры настроения, выбранные участниками)
    - 30% — персональные предпочтения (жанры лайкнутых фильмов)
    - 20% — рейтинг IMDb
    
    Логика:
    1. Собираем личные настройки ВСЕХ участников
    2. Жанры объединяются через OR (показываем фильмы по ЛЮБОМУ жанру)
    3. Длительность — минимум из всех (самое жёсткое ограничение)
    4. Расчёт скора с учётом трёх компонентов
    """
    try:
        ...
        # Подсчитываем "рейтинг" каждого фильма по голосам
        movie_scores = {}
        for v in votes:
            movie_scores[v.movie_id] = movie_scores.get(v.movie_id, 0) + v.vote
        
        # ID фильмов, за которые уже голосовали
        ...
        # 🔽 ШАГ 6: Собираем персональные предпочтения (лайки участников)
        ...
        for movie in candidates:
            ...
            # 🔽 КОМПОНЕНТ 1: Совпадение с критериями комнаты (50%)
            ...
            # 🔽 КОМПОНЕНТ 2: Совпадение с персональными предпочтениями (30%)
            ...
            # 🔽 КОМПОНЕНТ 3: Рейтинг IMDb (20%)
            rating_score = (movie.imdb_rating / 10.0) * 100 if movie.imdb_rating else 0
            
            # 🔽 ИТОГОВАЯ ФОРМУЛА: 50% + 30% + 20%
            final_score = round(
                room_genre_score * 0.5 + 
                personal_score * 0.3 + 
                rating_score * 0.2, 
                1
            )
        ...
        logger.info(
            ...
            f"найдено {len(results)} кандидатов, топ-10 возвращён"
        )
